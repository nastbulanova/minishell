...
t_extended_exec_data *mock_parser(char *input, t_minishell *data)
{
    ...
    cmd_1->opt = safe_malloc(3 * sizeof(char*), "mock_parser @ mock_parser.c");
    cmd_1->opt[0] = ft_strdup("-l");
    cmd_1->opt[1] = ft_strdup("-a");;
    cmd_1->opt[2] = NULL;
...
void execute_commands(t_extended_exec_data *commands, t_minishell *data)
{
    ...
    while (current)
    {
        t_extended_exec_data_print(current, false, true); //DEBUG print command

        if (!current->is_pipe_output && !current->outfile)
        {
            // No pipe or redirection, directly execute the command
            pid_t pid = fork();
